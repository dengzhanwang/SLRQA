...
check(~any(any(flip(A, 1) - s(flip(quaternion(A), 1)))), 'flip fails test 1')
check(~any(any(flip(A, 2) - s(flip(quaternion(A), 2)))), 'flip fails test 2')
...
check(~any(any(flip(A, 1) - part(flip(octonion(A), 1), 1))), 'flip fails test 1')
check(~any(any(flip(A, 2) - part(flip(octonion(A), 2), 1))), 'flip fails test 2')
...
% A = rando(4, 1);
% B = rando(1, 4);
% C = A * B; assert(all(size© == [4, 4]));
% 
% check(rank© == 1,          'rank fails test 1')
...
% $Id: test_matfuns.m 1114 2021-02-01 18:59:02Z sangwine $
