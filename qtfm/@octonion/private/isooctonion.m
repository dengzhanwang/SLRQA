...
if ~isnumeric(z) && ~isa(z, 'sym')
    error(['First argument to private function isooctonion must be ', ...
...
if any(any(U))
    q = subsasgn(q, substruct('()', {U}), ...
        octonion(z(U))); % A zero vector part will be supplied.
end
if any(any(D))
...
if ispure(a)
    % The octonion a is pure, so the result should be pure. This means,
    % for example, that sign(oi) would return oi and not a full octonion.
    q = vector(q);
end
...
% $Id: isooctonion.m 1125 2021-08-15 20:43:38Z sangwine $
