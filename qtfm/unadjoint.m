...
if strcmp(F, 'complex')
    ...
    if ~(isnumeric(A) || isa(A, 'sym'))
        error(['An adjoint matrix must be numeric or symbolic, given ', ...
                class(A)])
    end
...
elseif strcmp(F, 'real')
    ...
    % TODO This case applies only for complex quaternion matrices, so the
    % symbolic case here is more complex than above. If A is a quaternion
    % matrix, we need to be sure that it is complex, i.e. that it has no
    % assumptions applied that would make it real. Tricky to do.
...
if any(any(abs(RT - RA) > 1e-10)) || ...
   any(any(abs(IT - IA) > 1e-10))
...
% $Id: unadjoint.m 1113 2021-02-01 18:41:09Z sangwine $
