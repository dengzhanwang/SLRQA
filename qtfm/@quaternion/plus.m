...
% Copyright © 2005, 2008, 2010, 2016 Stephen J. Sangwine
...
%if ~strcmp(lc, rc)
    ...
    % stupid inconsistency. We should also permit double + sym here, or in
    % fact numeric + sym in either order.
    
    % January 2021 The check has been commented out, because it is very
    % complex to deal with the above TODO, and thinking about it, perhaps
    % it is better to let the function attempt to add the values, and hit
    % an error deeper in Matlab when it attempts to add incompatible types.
    % The only worry about this is that it performs an add which has
