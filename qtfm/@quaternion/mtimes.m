...
% 1 June 2020 Established that setting q = quaternion.empty is much more
% costly than the current code.
...
if ql && qr
    ...
    % One of the arguments is not a quaternion. If it is numeric, logical,
    % or symbolic, we can handle it.
    
    if ql && (isa(r, 'numeric') || isa(r, 'logical') || isa(r, 'sym'))
    ...
    elseif (isa(l, 'numeric') || isa(l, 'logical') || isa(l, 'sym')) && qr
        ...
        if ql
            error(['Matrix multiplication of a quaternion by a ', ...
                    class(r), ' is not possible.'])
        else
            error(['Matrix multiplication of a ', class(l), ...
                   ' by a quaternion is not possible.'])
        end
    end
end
...
% $Id: mtimes.m 1127 2021-08-30 19:35:41Z sangwine $
