...
while true % This is really a repeat .. until, but since Matlab does not
  ...
  for r = 1 : N - 1
    for c = r + 1 : N
      ...
      b_rr = sum(abs(A(:,r)).^2); % Real value. Note 1.
...
% Note 1: Taking the absolute value (modulus) requires a square root, which
% is then followed by squaring. Why? Because there is no single function
% which can compute the norm of a real, complex or quaternion value. The
% function normq does the job for quaternions without the square root, but
% it won't work for real or complex values. We could put in a select/case
% here to handle the three cases. But it seems better to just use abs and
% squaring which works for all three without the complexity.

% $Id: svdj.m 1116 2021-02-01 19:11:37Z sangwine $
